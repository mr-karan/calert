...
|  `providers.<room_name>.timeout` 	| Timeout for making HTTP requests to the webhook URL.  	| `30s` |
...
|  `providers.<room_name>.proxy_url` 	| Specify `proxy_url` as your proxy endpoint to route all HTTP requests to the provider via a proxy. | - |
|  `providers.<room_name>.threaded_replies` 	| Whether to send threaded replies or not. | false |
|  `providers.<room_name>.dry_run` 	| In case you're simply experimenting with `calert` config changes and you don't wish to send _actual_ notifications, you can set true. | false |
|  `providers.<room_name>.retry_max` 	| Maximum number of retries | `3` |
|  `providers.<room_name>.retry_wait_min` 	| Minimum time to wait before retrying | `1s` |
|  `providers.<room_name>.retry_wait_max` 	| Maximum time to wait before retrying | `5s` |
